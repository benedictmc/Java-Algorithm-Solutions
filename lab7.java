...
//create the hash table
    ...
    public static void main (String[] args) throws IOException {
      File testFile = new File("X:/Users/Athbheocain/Desktop/dictionary.txt");     //this is where the file to be sorted is loaded from
...
//the whole thing is added together and then moduloed to create a hash table index
		int hashValue =0, count =0, hash =0;
		int sum=0;
		char[] c_array = word.toCharArray();

		for(int i = 0; i < c_array.length; i++){
			double num = (double)c_array[i];
		...
		for (int i = 0; i < word.length(); i++){
		    hash += (int)c_array[i]*modPow(258, count, 402001);
		    count++;
		    sum += (int)c_array[i];
		}
		hash = hash % size;
...
//for example, you could just add up all of the letters in the word
		...
		for(char c : word.toCharArray()) {
			i = ((int) c);
		    hash += i*modPow(258, count, 402001);
		    count++;
		}
		hash = hash % size;
